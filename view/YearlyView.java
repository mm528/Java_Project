...
public class YearlyView extends JFrame{
	...
	JLabel labelW = new JLabel("Mondaykjsdbcjbsdcbdjks");
	...
	JButton weekly = new JButton ("Weeklynb hj hj hj h h h");
	...
	static JFrame wind = new JFrame("Monthly Viewekjsdbck djks ");
 ...
 private final JButton btnNotifications = new JButton("NotificationMike");
	...
	public YearlyView(){
		...
		wind.setSize(570,400);
		wind.setLocation(500,200);
		wind.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);//no keep all windows open
		...
		try {
			URL url = new URL("http://png-1.findicons.com/files/icons/1786/oxygen_refit/128/zoom_in.png");
		...
		try {
			...
			b2= new JButton(new ImageIcon(((img).
					getImage()).getScaledInstance(40, 40, java.awt.Image.SCALE_SMOOTH)));
		...
		try {
			...
			eVent= new JButton(new ImageIcon(((img).
					getImage()).getScaledInstance(40, 40, java.awt.Image.SCALE_SMOOTH)));
		...
		p.add(eVent);
		  ...
		  try {
				...
				deleteEvent= new JButton(new ImageIcon(((img).
				 		getImage()).getScaledInstance(40, 40, java.awt.Image.SCALE_SMOOTH)));
...
/////////////////////////////////////////////////////////////////		
	...
	//CmbUserSelect.setVisible(superuser);
	...
	public JTable table(){
		...
		String columnName[] = {"Jan","Feb","Mar","Apr","May","June"};
	
		/**
		for (int i=0; i <columnName.length-1;i++){
			if (calPage <4 && click ){
				columnName[i]= columnName[i] +7;
				table.repaint();
			}
		}
**/
	...
	//			
		...
		JTableHeader header = table.getTableHeader();//get the head and then change color
	...
	public JTableHeader header() {
		...
		if (calPage == 0){
      		for (int i=0; i<=5; i++){
      			String[] headers = {"Jan", "Feb", "Mar", "Apr", "May", "Jun"}; //All headers
      		...
      		for (int i=0; i<=5; i++){
               String[] headers = {"Jul", "Aug", "Sept", "Oct", "Nov", "Dec"};
      			TableColumnModel tcm = header.getColumnModel();
    			javax.swing.table.TableColumn tc = tcm.getColumn(i);
    			tc.setHeaderValue(headers[i]);
    			header.setFont(new Font("Dialog", Font.CENTER_BASELINE, 10));
